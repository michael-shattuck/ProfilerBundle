...
class XHProfReport extends XHProfLib
{
    ...
    function getReport($url_params)
    {
        ...
        if (!empty($url_params['symbol'])) {
            $xhprof_data = $this->xhprof_trim_run($xhprof_data, array($url_params['symbol']));
        }
        ...
        return $this->full_report(new Report(), $symbol_tab, $url_params['all']);
    }
     ...
     */
    function full_report(Report $report, $symbol_tab, $limit)
    {
        ...
        if ($limit > 0) {
            ...
            for ($i = 0; $i < $limit; $i++) {
                $this->getFunctionInfo($report, $flat_data[$i]);
            }
        }
        else if ($limit < 0) {
            ...
            for ($i = 0; $i < $limit; $i++) {
                $this->getFunctionInfo($report, $flat_data[$size - $i - 1]);
            }
        }
        else {
            foreach ($flat_data as $func) {
                $this->getFunctionInfo($report, $func);
            }
        }
    ...
    protected function getFunctionInfo(Report $report, $func)
    {

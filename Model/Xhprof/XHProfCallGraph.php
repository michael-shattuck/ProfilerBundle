...
use Clamidity\ProfilerBundle\Model\Xhprof\XHProf;
...
class XHProfCallGraph extends XHProf
{
    ...
    */
    ...
    {
        if (!$name) {
            $this->xhprof_error('http_header usage');
            return null;
        }

        if (!is_string($value)) {
            $this->xhprof_error('http_header value not a string');
        }
    ...
    */
    function xhprof_generate_mime_header($type, $length)
    {
        ...
        if ($mime) {
            $this->xhprof_http_header('Content-type', $mime);
            $this->xhprof_http_header('Content-length', (string) $length);
        }
    }
    ...
    */
    function xhprof_generate_dot_script($raw_data, $threshold, $source, $page, $func, $critical_path, $right = null, $left = null)
    {
        ...
        $sym_table = $this->xhprof_compute_flat_info($raw_data, $totals);
            ...
            while ($node) {
                ...
                if (isset($children_table[$node])) {
                    ...
                    foreach ($children_table[$node] as $child) {
                        ...
                        if ($max_child === null ||
                                abs($raw_data[$this->xhprof_build_parent_child_key($node, $child)]["wt"]) >
                                abs($raw_data[$this->xhprof_build_parent_child_key($node, $max_child)]["wt"])) {
                            $max_child = $child;
                        }
                    }
                    if ($max_child !== null) {
            ...
            if (isset($sym_table[$parent]) && isset($sym_table[$child]) &&
                ...
                $headlabel = $sym_table[$child]["wt"] > 0 ?
                        sprintf("%.1f%%", 100 * $info["wt"]
                                / $sym_table[$child]["wt"]) : "0.0%";

                $taillabel = ($sym_table[$parent]["wt"] > 0) ?
                        sprintf("%.1f%%", 100 * $info["wt"] /
                                ($sym_table[$parent]["wt"] - $sym_table["$parent"]["excl_wt"])) : "0.0%";
    ...
    function xhprof_render_diff_image($params)
    {
        $total1;
        $total2;

        $raw_data1 = $this->get_run($params['run1'], $params['source'], $params['desc_unused']);
        $raw_data2 = $this->get_run($params['run2'], $params['source'], $params['desc_unused']);

        // init_metrics($raw_data1, null, null);
        $children_table1 = $this->xhprof_get_children_table($raw_data1);
        $children_table2 = $this->xhprof_get_children_table($raw_data2);
        $symbol_tab1 = $this->xhprof_compute_flat_info($raw_data1, $total1);
        $symbol_tab2 = $this->xhprof_compute_flat_info($raw_data2, $total2);
        $run_delta = $this->xhprof_compute_diff($raw_data1, $raw_data2);
        $script = $this->xhprof_generate_dot_script($run_delta, $params['threshold'], $params['source'], null, null, true, $symbol_tab1, $symbol_tab2);
        $content = $this->xhprof_generate_image_by_dot($script, $params['type']);

        $this->xhprof_generate_mime_header($params['type'], strlen($content));
    ...
    * @author cjiang
    */
    function xhprof_get_content_by_run($run_id, $type, $threshold, $func, $source, $critical_path)
    {
        if (!$run_id)
            return "";

        $raw_data = $this->get_run($run_id, $source, $description);
        if (!$raw_data) {
            $this->xhprof_error("Raw data is empty");
            return "";
        }

        $script  = $this->xhprof_generate_dot_script($raw_data, $threshold, $source, $description, $func, $critical_path);
        $content = $this->xhprof_generate_image_by_dot($script, $type);
    ...
    * @author cjiang
    */
    function xhprof_render_image($params)
    {
        $content = $this->xhprof_get_content_by_run($params['run'], $params['type'], $params['threshold'], $params['func'], $params['source'], $params['critical']);

        if (!$content) {
            print "Error: either we can not find profile data for run_id " . $params['run_id']
                    . " or the threshold " . $params['type'] . " is too small or you do not"
